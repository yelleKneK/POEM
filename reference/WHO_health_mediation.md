...
manuscript. It is a country-by-year panel used to ask how health-care
...
The five health outcomes (`imr`, `u5mr`, `leb`) cover all 91 members for
2000–2021; `lbw` and `pou` have narrower coverage, so they carry `NA`
where unavailable. The mediators are intercorrelated by construction,
since many are different normalizations of the same underlying spending
(per capita, percent of GDP, percent of current health expenditure,
constant versus current currency, and so on), which is precisely the
high-dimensional, correlated-mediator regime the methods target.
...
In the manuscript's global model the confounders are the region, income
group, and year.
...
# Reproduce the manuscript's global infant-mortality analysis.
...
#>  j_pe                  149     
#>  pval_pe               < 0.0001
#>  total_indirect_effect 0.1104  
#>  total_indirect_lower  -0.2361 
#>  total_indirect_upper  0.4569  
